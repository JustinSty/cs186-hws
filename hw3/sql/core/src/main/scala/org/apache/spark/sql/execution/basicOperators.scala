...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    val partitions_iterator = partitions.getIterator
    var generator: (Iterator[Row] => Iterator[Row]) = CS186Utils.generateCachingIterator(projectList, child.output)
    var current_partition: DiskPartition = partitions_iterator.next
    var currentIterator: Iterator[Row] = generator(current_partition.getData)
    ...
    new Iterator[Row] {
      ...
      private def fetchNextPartition(): Boolean  = {
        // IMPLEMENT ME
        if (partitions_iterator.hasNext) {
          current_partition = partitions_iterator.next
          currentIterator = generator(current_partition.getData)
