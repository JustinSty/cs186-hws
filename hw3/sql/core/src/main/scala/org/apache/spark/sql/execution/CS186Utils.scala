      ...
      new Iterator[Row] {
        ...
        def next() = {
          ...
          if (input.hasNext) {
            ...
            val udf = udfProject(row)
            val pre = preUdfProjection(row)
            val post = postUdfProjection(row)
            ...
            if (cache.containsKey(cachekey)) {
              ...
              if (pre.length < 1 && post.length < 1) {
                ...
                concat = Row(udf(0),post(0))
              } else if (post.length < 1) {
                concat = Row(pre(0), udf(0))
              } else {
                concat = Row(pre(0), udf(0), post(0))
              }
              cache.put(cachekey, concat)
              result = cache.get(cachekey)
            }
          } else {
            throw new RuntimeException(s"Accessing out of iterator without next")
          }
          //ok
          result
        }
