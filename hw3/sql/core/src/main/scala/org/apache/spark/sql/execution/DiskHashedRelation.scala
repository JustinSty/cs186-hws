...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    for (i <- 0 to (size-1)){
      partitions(i) = new DiskPartition("part"+i,blockSize)  
    }
    for (row <- input){
      //var roww: Row = input.next()
      var partitionNum = keyGenerator(row).hashCode() % size    //generate key for each row, get hash code, take the modulo by the number of the partitions 
      partitions(partitionNum).insert(row)      //insert into the partition 
