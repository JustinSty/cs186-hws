...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        // TODO: IMPLEMENT ME
        if(joinOp == Predicate.Op.EQUALS || joinOp == Predicate.Op.NOT_EQUALS || joinOp == Predicate.Op.LIKE) {
            if (t1pkey && t2pkey){
            ...
            return (int) (0.1*card1*card2);    // assume 1/10 
        }
    ...
    public Vector<LogicalJoinNode> orderGreedyJoins(
        ...
        while (!joinsleft.isEmpty()) {
            ...
            LogicalJoinNode join_to_remove = new LogicalJoinNode();
            for (LogicalJoinNode join : joinsleft) {
                // System.out.print("cur join: ");
                // System.out.print(join);
                CostCard cc = costGreedyJoin(join, plan, planCardinalities, planCosts, stats, filterSelectivities);
                // System.out.print(" cost: ");
                // System.out.println(cc.cost);
                if (cc != null && cc.cost < bestplan.cost) {
                    bestplan = cc;
                    join_to_remove = join;
                }
            }
            // System.out.print("join to remove: ");
            // System.out.println(join_to_remove);
            plan.add(join_to_remove);
            planCardinalities.add(bestplan.card);
            planCosts.add(bestplan.cost);
            joinsleft.remove(join_to_remove);
    ...
    public Vector<LogicalJoinNode> orderDynamicProgrammingJoins(
        ...
        for (int i = 0; i < joins.size(); i++) {
            ...
            Set<Set<LogicalJoinNode>> len_i_joins = enumerateSubsets(joins, i + 1);
            // System.out.println(len_i_joins);
            for (Set<LogicalJoinNode> len_i_join : len_i_joins) {      // s
                CostCard bestplan = new CostCard();
                bestplan.cost = Double.MAX_VALUE;
                for (LogicalJoinNode e : len_i_join) {  
                    Set<LogicalJoinNode> subjoin = new HashSet<LogicalJoinNode>();
                    subjoin.addAll(len_i_join);
                    subjoin.remove(e);  //s'
                    CostCard cur_cc = computeCostAndCardOfSubplan(stats, filterSelectivities, e, len_i_join, bestplan.cost, pc);
                    // System.out.print("cur join: ");
                    // System.out.print(subjoin);
                    // System.out.print(" and: ");
                    // System.out.println(e);
                    if (cur_cc != null && cur_cc.cost < bestplan.cost) {
                        bestplan = cur_cc;
                        // Vector<LogicalJoinNode> bplan = pc.getOrder(subjoin);
                        // bplan.add(e);
                        // bestplan.plan = bplan;
                        // System.out.print("cur_plan: ");
                        // System.out.println(cur_cc.plan);
                        // System.out.print("cur_cost: ");
                        // System.out.println(cur_cc.cost);
                    }
                }
                if (bestplan.plan != null) {
                    pc.addPlan(len_i_join, bestplan.cost, bestplan.card, bestplan.plan);
                    // System.out.println("end for ========================");
                    // System.out.print("best join:");
                    // System.out.println(len_i_join);
                    // System.out.print("plan: ");
                    // System.out.println(bestplan.plan);
                    // System.out.print("Cost:");
        ...
        Set<Set<LogicalJoinNode>> set_set_joins = enumerateSubsets(joins, joins.size());
        Vector<LogicalJoinNode> result = new Vector<LogicalJoinNode>();

        for (Set<LogicalJoinNode> set_joins : set_set_joins) {
            result = pc.getOrder(set_joins);
            break;
        }
