 ...
 * log records (on page flushed) and the log file flushes BufferP
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                ArrayList<PageId> pagess = new ArrayList<PageId>();
               
                long end = raf.length();
                raf.seek(firstLogRecord);

                while (raf.getFilePointer() < end){
                    int xactType = raf.readInt();
                    long xactId = raf.readLong();
                    if (xactType == CHECKPOINT_RECORD){
                    ...
                    else if (xactType == UPDATE_RECORD){
                        Page before = readPageData(raf);
                        Page after = readPageData(raf);    
                        if (tid.getId()== xactId){
                            if (! pagess.contains(before.getId())){
                                pagess.add(before.getId());
            ...
            raf.seek(raf.getFilePointer());  /** After you are done with rollback, make sure that your raf file pointer is at the 
            end of the last complete log record, currentOffset **/
            }
        }
    }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                HashMap<Long,Page> tidToAfter = new HashMap<Long,Page>();
                HashMap<Long,Page> tidToBefore = new HashMap<Long,Page>();
                ArrayList<PageId> pagess = new ArrayList<PageId>();
                long last_tid = -1;
                int last_t_commit = 0;
                ...
                while (raf.getFilePointer() < end){
                    ...
                    if (xactType == CHECKPOINT_RECORD){
                        ...
                        Page before = readPageData(raf);
                        Page after = readPageData(raf);   
                        tidToAfter.put(xactId, after);
                        if (!tidToBefore.keySet().contains(xactId)) {
                            tidToBefore.put(xactId, before);
                        }
                    } else if (xactType == COMMIT_RECORD) {
                        tidToBefore.remove(xactId);
                    } else if (xactType == ABORT_RECORD) {
                        Page before = tidToBefore.get(xactId);
                        Database.getCatalog().getDbFile(before.getId().getTableId()).writePage(before);
                        tidToBefore.remove(xactId);
                        tidToAfter.remove(xactId);
                    }
                raf.readLong();
                }
                for (long id : tidToAfter.keySet()) {
                    Page after = tidToAfter.get(id);
                    Database.getCatalog().getDbFile(after.getId().getTableId()).writePage(after);
                }
                for (long id : tidToBefore.keySet()) {
                    Page before = tidToBefore.get(id);
                    Database.getCatalog().getDbFile(before.getId().getTableId()).writePage(before);
                }
            }
        }
